...
// class subnet
subnet::subnet(int hosts_) // constructor for given number of hosts, will not be used
{
...
subnet::subnet(char input[]) // constructor for char-array
{
	char byte1[3] = {0};
	char byte2[3] = {0};
	char byte3[3] = {0};
	...
	this->set_ip_first(atoi(byte1), atoi(byte2), atoi(byte3), atoi(byte4)); //set ip_first to input
...
{
	char output[4000];
	snprintf(output, 4000, "Network: %i.%i.%i.%i/%i\nBroadcast: %i.%i.%i.%i\nNetmask: %i.%i.%i.%i\nHosts: %i\nUsable Addresses: %i\n\n\n",
